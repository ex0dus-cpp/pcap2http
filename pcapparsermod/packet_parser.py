...
import struct
import socket
from pcapparsermod.constant import *
from pcapparsermod.link_layer import LinkLayer
from pcapparsermod import packet_counter


class TcpPack:
    """ a tcp packet, header fields and data. """

    def __init__(self, source, source_port, dest, dest_port, flags, seq, ack_seq, body, number):
        self.source = source
...
def read_tcp_packet(read_packet):
    ...
    for link_type, micro_second, link_packet in read_packet():
        ...
        if transport_protocol == TransferProtocol.TCP:
            ...
            yield TcpPack(source, source_port, dest, dest_port, flags, seq, ack_seq, body, packet_counter.get())
